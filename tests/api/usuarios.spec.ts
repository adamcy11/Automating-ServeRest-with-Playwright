...
test.describe('GET /usuarios', () => {
  ...
  test('should get a user by id', async ({ apiAsAdmin }) => {
    ...
    expect(response.status()).toBe(200)
    expect(body.nome, 'Returned name should match the one sent during creation').toBe(newUser.nome)
    expect(body.email, 'Returned email should match the one sent during creation').toBe(newUser.email)
    expect(body.administrador, 'Administrator flag should be preserved after creation').toBe(newUser.administrador)
  })
  ...
  test('should return a validation error when id has invalid format', async ({ apiAsAdmin }) => {
    ...
    expect(response.status(), 'API should reject malformed ids before attempting to look them up').toBe(400)
  ...
  test('should return an error when user id does not exist', async ({ apiAsAdmin }) => {
    ...
    expect(response.status(), 'API should return a business error for well-formed ids that do not exist').toBe(400)
